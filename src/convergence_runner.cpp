...
{
  double
  safe_order(const double e_coarse, const double e_fine, const double x_coarse, const double x_fine)
  {
    if (e_coarse <= 0.0 || e_fine <= 0.0 || x_coarse <= 0.0 || x_fine <= 0.0 || std::abs(x_coarse - x_fine) < 1e-14)
      return 0.0;
...
ConvergenceRunner::ConvergenceRunner(const WaveProblemConfig &config, MPI_Comm mpi_communicator)
  : base_config(config)
...
ConvergenceRunner::run_space_study() const
{
  ...
  std::sort(entries.begin(), entries.end(), [](const Entry &a, const Entry &b) { return a.h > b.h; });

  for (std::size_t i = 1; i < entries.size(); ++i)
    {
      entries[i].observed_order_l2 =
        safe_order(entries[i - 1].l2_error, entries[i].l2_error, entries[i - 1].h, entries[i].h);
      entries[i].observed_order_h1 =
        safe_order(entries[i - 1].h1_error, entries[i].h1_error, entries[i - 1].h, entries[i].h);
    }
...
ConvergenceRunner::run_time_study() const
{
  ...
  std::sort(entries.begin(), entries.end(), [](const Entry &a, const Entry &b) { return a.dt > b.dt; });

  for (std::size_t i = 1; i < entries.size(); ++i)
    {
      entries[i].observed_order_l2 =
        safe_order(entries[i - 1].l2_error, entries[i].l2_error, entries[i - 1].dt, entries[i].dt);
      entries[i].observed_order_h1 =
        safe_order(entries[i - 1].h1_error, entries[i].h1_error, entries[i - 1].dt, entries[i].dt);
    }
...
ConvergenceRunner::write_csv(const std::string &path, const std::vector<Entry> &entries) const
{
  ...
  out << "study,method,fe_degree,theta,beta,gamma,mesh_file,dt,n_steps,t_final,h,ndofs,l2_error,h1_error,observed_"
         "order_l2,observed_order_h1\n";

  const std::string method_name = (base_config.method == WaveProblemConfig::Method::theta) ? "theta" : "newmark";

  out << std::setprecision(14);
  for (const auto &e : entries)
    {
      out << e.study << ',' << method_name << ',' << base_config.fe_degree << ',' << base_config.theta << ','
          << base_config.beta << ',' << base_config.gamma << ',' << e.mesh_file << ',' << e.dt << ',' << e.n_steps
          << ',' << e.t_final << ',' << e.h << ',' << e.ndofs << ',' << e.l2_error << ',' << e.h1_error << ','
