...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.82\textwidth]{../solution/convergence.png}
...
mpirun -n 4 ./cmake-build-release/wave-equation \
  --config configs/theta_crank_nicolson.cfg
\end{verbatim}

\subsection{Convergence + Plot}
\begin{verbatim}
mpirun -n 4 ./cmake-build-release/wave-equation \
  --config configs/convergence_both.cfg
python3 scripts/plot_convergence.py \
  --space-csv solution/convergence_space.csv \
  --time-csv solution/convergence_time.csv \
  --output solution/convergence.png
\end{verbatim}
